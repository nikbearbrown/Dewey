...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - Dewey Tools`,
      description: doc.description || `${doc.title} — tool by Dewey`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - Dewey Tools`,
        description: rows[0].description || `${rows[0].name} — tool by Dewey`,
      }
    }
  } catch {}

  return { title: 'Tool - Dewey' }
}
